...
lock.prototype.renderDom = function () {
  ...
  let canvasWidth = this.getCanvasWidth(this.size)
  let lock_wrap = document.createElement('div')
  let str = '<h4 class="lock_title">' + this.title + '</h4>' +
...
lock.prototype.moveUpdate = function (pos) {
  for (let i = 0; i < this.leftCircle.length; i++) {
...
lock.prototype.endUpdate = function (psw) {
  //  获取提示栏
  let lock_warn = document.getElementsByClassName('lock_warn')[0]
  if (this.state === inputPsw) {
    ...
    let pswStore = JSON.parse(window.localStorage.getItem('password'))
...
lock.prototype.bindEvent = function () {
  ...
  this.lock_panel.addEventListener('touchstart', function (e) {
    e.preventDefault()
    //  获取触摸位置
    let pos = self.getPosition(e)
    //  判断触摸的是哪个圆圈
    for (let i = 0; i < self.circleArr.length; i++) {
